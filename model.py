...
class SentimentRecommenderModel:
    ...
    ROOT_PATH = "pickle/"
    MODEL_NAME = "sentiment-based-recommendation-xg-boost-model.pkl"
    ...
    # Get the top product 20 recommendations for the user"""
    def getRecommendationByUser(self, user):
        recommedations = []
        return list(self.user_final_rating.loc[user].sort_values(ascending=False)[0:20].index)

    # Filter the product recommendations using the sentiment model and get the top 5 recommendations"""
    ...
    # Classify the sentiment to 1/0 - positive or negative - using the trained ML model
    ...
    # Preprocess the text before it's sent to ML model
    def preprocess_text(self, text):
     ...
     # Get the pos tag to derive the lemma form
    ...
    # Derive the base lemma form of the text using the pos tag
